...
def user_input_features():
    # (label,minvalues,maxvalues,initial values)
    starRating = st.sidebar.slider('Star Rating', 0, 5, 3)
    builtYear = st.sidebar.slider('Built Year', 1900, 2023, 1960)
    # (label,minvalues,maxvalues,initial values,jarak increment,format spesifik)
    ...
    roomFacilitie = st.sidebar.multiselect(
        'Room Faciclities', (roomFacilities))
...
df = pd.concat(
    [df, hotelFacilities_df, roomFacilities_df, nearestPoint_df], axis=1)
...
if df.columns.tolist() == colOri.all():
    ...
    mismatched_columns = [(idx, df_col, model_col) for idx, (df_col, model_col) in enumerate(
        zip(df.columns.tolist(), colOri)) if df_col != model_col]

    if len(mismatched_columns) > 0:
        st.warning(
            "The order of the columns is not the same as the model. Mismatched columns:")
        for idx, df_col, model_col in mismatched_columns:
            st.write(
                f"At index {idx}: DataFrame column '{df_col}' - Model column '{model_col}'")
